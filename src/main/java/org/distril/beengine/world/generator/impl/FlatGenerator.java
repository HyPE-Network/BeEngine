...
public class FlatGenerator implements Generator {
	...
	public void generate(ThreadLocalRandom random, Chunk chunk) {
		for (int x = 0; x < 16; x++) {
			for (int z = 0; z < 16; z++) {
				chunk.setBlock(x, 0, z, 0, Material.BEDROCK.getBlock());
				chunk.setBlock(x, 1, z, 0, Material.DIRT.getBlock());
				chunk.setBlock(x, 2, z, 0, Material.DIRT.getBlock());
				chunk.setBlock(x, 3, z, 0, Material.GRASS.getBlock());
			}
		}
	}
}
